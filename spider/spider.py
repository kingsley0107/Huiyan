...
from configs.static_vars import POI_SAVE_PATH, YEARS, CITIES, POI_TYPE_ONE, POI_TYPE_TWO
    ...
    for city in CITIES:
        for year in YEARS:
            task = asyncio.ensure_future(
                crawler.crawl_poi2file(city, year, POI_TYPE_ONE, POI_TYPE_TWO, save=True)
            )
            tasks.append(task)
